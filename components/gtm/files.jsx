    ...
    <div className="scroll" style={{ flex: 1 }}>
      <div className="screen-pad" style={{ maxWidth: 900, margin: "0 auto" }}>
        ...
        >
          <div style={{ flex: 1, minWidth: 220 }}>
            ...
            <p style={{ fontSize: 14, color: "var(--fg-muted)", maxWidth: 560 }}>
              Files you upload in chat become knowledge the agent can read. Attach a deal or account
              first to link the file to it.
            </p>
          </div>
          <button className="btn btn-sm btn-primary" onClick={() => onNewChat([])}>
            <Icons.Plus size={14} /> Upload in a new chat
          </button>
        </div>
        ...
        {files.length === 0 ? (
          <div
            className="card"
            style={{ padding: 44, textAlign: "center", color: "var(--fg-muted)" }}
          >
            <Icons.Paperclip size={28} style={{ marginBottom: 10 }} />
            <div style={{ fontSize: 14 }}>
              No files yet. Open a chat and use the clip in the composer to upload one.
            </div>
          </div>
        ) : (
              ...
              >
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 12.5, color: "var(--fg-muted)" }}>
                    {f.linkedName ? `Linked to ${f.linkedName} · ` : ""}
                    {fmtTime(f.ts)}
                    {f.datasourceId == null ? "" : ` · datasource ${f.datasourceId}`}
                  </div>
                </div>
                <span

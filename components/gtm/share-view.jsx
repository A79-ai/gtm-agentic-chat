...
export function ShareView({ shareId }) {
  ...
  useEffect(() => {
    ...
    el.dataset.accent = "gold";
    let alive = true;
    fetch(`/api/share/${shareId}`)
      .then((r) => (r.ok ? r.json() : Promise.reject(new Error(String(r.status)))))
      .then((d) => {
        if (alive) {
          setState({ loading: false, transcript: d.transcript });
        }
      })
      .catch(() => {
        if (alive) {
        ...
        <div className="share-brand">
          ...
          <div style={{ minWidth: 0 }}>
            ...
            <div className="share-sub">
              <Icons.Share size={11} /> Read-only shared conversation
            </div>
          </div>
        </div>
        ...
        <div className="chat-inner">
          {state.loading ? (
            <div className="share-empty">
              <Icons.Refresh className="spin" size={20} />
              <div style={{ marginTop: 10 }}>Loading conversation…</div>
            </div>
          ) : state.error ? (
            <div className="share-empty">
              <Icons.Inbox size={26} />
              <div style={{ marginTop: 10 }}>This shared link is invalid or has expired.</div>
            </div>
          ) : messages.length === 0 ? (
            <div className="share-empty">
              <Icons.Inbox size={26} />
              <div style={{ marginTop: 10 }}>This conversation is empty.</div>
            </div>
          ) : (
            messages.map((m, i) =>
              m.role === "user" ? (
                <div className="msg-row msg-user" key={i}>
                  <div className="bubble">{m.text}</div>
                </div>
              ) : (
                <div className="msg-row msg-agent" key={i}>
                  <div className="msg-avatar" style={{ color: "var(--fg-primary)" }}>
                    <LogoMark size={17} />
                  </div>
                  <div className="bubble" style={{ flex: 1, minWidth: 0, fontSize: 14.5 }}>

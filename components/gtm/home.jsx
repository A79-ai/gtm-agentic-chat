    ...
    <div className={"card" + (hover ? " card-hover" : "")} onClick={onOpen}
      ...
      <div style={{ padding: 20, display: "flex", flexDirection: "column", gap: 14, flex: 1 }}>
        <div style={{ display: "flex", alignItems: "flex-start", gap: 12 }}>
          ...
          {agent.starter ? <TBadge value="Starter" tone="accent" /> : !agent.builtin ? <TBadge value="Custom" tone="neutral" /> : null}
        </div>
      ...
      <div style={{ borderTop: "1px solid var(--border-subtle)", padding: "12px 16px", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          {tools.length > 0 ? (
            ...
            <span style={{ fontSize: 11.5, color: "var(--fg-muted)", display: "inline-flex", alignItems: "center", gap: 5 }}><Icons.Plug size={12} /> {serverCount} source{serverCount === 1 ? "" : "s"}</span>
          )}
        </div>
        {agent.lastRun && <span style={{ fontSize: 11.5, color: "var(--fg-muted)", display: "inline-flex", alignItems: "center", gap: 4 }}><Icons.Clock size={12} /> {agent.lastRun}</span>}
      </div>
    </div>
  );
}

...
import { byId, COLUMNS, CONNECTORS, ENTITIES, recordsOf, subtitleOf } from "@/lib/gtm/data";
import { Icons } from "./icons";
import { Avatar, EntityIcon, RefChip, TBadge } from "./ui";
...
function pickOptions(kind) {
  ...
  if (kind === "stage") {
    const seen = [];
    recordsOf("deal").forEach((d) => {
      if (d.stage && !seen.includes(d.stage)) {
        seen.push(d.stage);
      }
    });
  ...
  if (kind === "owner") {
    return recordsOf("owner").map((o) => [o.id, o.name]);
  }
  return [];
}
  ...
  return (
    <div className="sel-picker" ref={ref}>
      ...
      <div className="sel-picker-list">
        {options.length === 0 ? (
          <div className="sel-picker-empty">No options</div>
        ) : (
          options.map(([v, l]) => (
            <button className="sel-picker-item" key={v} onClick={() => onPick(v)}>
              {l}
            </button>
          ))
        )}
...
function EntityPicker({ searchType, valueLabel, onPick }) {
  ...
  if (valueLabel) {
    return (
      <button className="create-picked" onClick={() => onPick(null)} type="button">
    ...
    <div className="create-picker">
      ...
      {open && q.trim() && (
        <div className="create-picker-list">
          {loading ? (
            <div className="sel-picker-empty">Searching…</div>
          ) : items.length === 0 ? (
            <div className="sel-picker-empty">No matches</div>
          ) : (
            items.map((it) => (
              <button
                className="sel-picker-item"
                key={it.id}
                onClick={() => {
                  onPick({ id: it.id, label: it.name });
                  setOpen(false);
                  setQ("");
                }}
                type="button"
...
function CreateModal({ type, onClose, onCreated, onToast }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        onToast(`${meta.label} created`, "success");
        onCreated && (await onCreated());
        onClose();
      } else {
        onToast(`Couldn't create ${meta.label.toLowerCase()}: ${res.error || "error"}`, "error");
      }
    } catch {
  ...
  const renderField = (f) => {
      ...
      return (
        <select
          ...
          onChange={(e) => {
            ...
            if (f.key === "parent_type") {
              set("parent_id", "");
              setLabels((l) => ({ ...l, parent_id: "" }));
            }
          }}
          value={vals[f.key] || ""}
        >
    ...
    if (f.kind === "entity" || f.kind === "parent") {
      const searchType =
        f.kind === "parent" ? PARENT_SEARCH[vals.parent_type || "opportunity"] : f.searchType;
      return (
        <EntityPicker
          onPick={(p) => {
            set(f.key, p ? p.id : "");
            setLabels((l) => ({ ...l, [f.key]: p ? p.label : "" }));
          }}
          searchType={searchType}
          valueLabel={labels[f.key]}
        />
      );
    }
    return (
  ...
  return (
    ...
    >
      <div
        ...
        style={{ width: "min(460px, 92vw)", maxHeight: "88vh", overflow: "auto", padding: 0 }}
      >
        <div
          style={{
            display: "flex",
        ...
        <div style={{ padding: 16, display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div style={{ display: "flex", gap: 8, justifyContent: "flex-end", marginTop: 4 }}>
            <button className="btn btn-sm btn-outline" disabled={busy} onClick={onClose}>
              Cancel
            </button>
            <button className="btn btn-sm btn-primary" disabled={missing || busy} onClick={submit}>
              {busy ? (
                <>
                  <Icons.Refresh className="spin" size={14} /> Creating…
                </>
              ) : (
                <>
                  <Icons.Plus size={14} /> Create {meta.label.toLowerCase()}
                </>
              )}
            </button>
          </div>
...
function MobileCard({ rec, selected, selecting, onToggle, onOpen }) {
  ...
  return (
    ...
    >
      ...
      <EntityIcon size={40} type={rec.type} />
      <div className="meta">
...
function SelectionBar({ count, label, plural, actions, pending, onAction, onChat, onClear }) {
  ...
  return (
    <div className="sel-bar">
      ...
      <button className="sel-act primary" disabled={pending} onClick={onChat}>
        <Icons.Spark size={14} /> Chat with {count} {noun}
      </button>
      {actions.map((a) => (
        <button className="sel-act" disabled={pending} key={a.id} onClick={() => onAction(a)}>
...
export function EntityList({ type, onOpen, onChat, onToast, onRefresh }) {
  ...
  const [visible, setVisible] = useState(() => {
    try {
      const s = JSON.parse(localStorage.getItem("ampup-cols-" + type));
      if (Array.isArray(s)) {
        return new Set([cols[0][0], ...s]);
      }
    } catch {}
  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      const url =
        `/api/list?type=${type}&page=${page}&size=${SIZE}` +
  ...
  const ownerRows = isOwner
    ? recordsOf("owner").filter(
        (r) =>
          !search ||
          r.name.toLowerCase().includes(search.toLowerCase()) ||
          subtitleOf(r).toLowerCase().includes(search.toLowerCase())
      )
    : [];
  ...
  const grid = `30px minmax(160px, 1.8fr) ${shownCols
    .slice(1)
    .map(() => "1fr")
    .join(" ")} 28px`;
  ...
  const toggleCol = (key) =>
    setVisible((prev) => {
      ...
      try {
        localStorage.setItem("ampup-cols-" + type, JSON.stringify([...next]));
      } catch {}
      return next;
    });
  ...
  const runBulk = async (action, value) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        onToast &&
          onToast(
            `${action.label}: ${res.done || 0}/${res.total || ids.length} updated`,
            res.done ? "info" : "error"
          );
      }
  ...
  const bulkAction = (a) => {
    if (a.pick) {
      setPicker({ action: a, options: pickOptions(a.pick) });
    } else {
      runBulk(a);
    }
  };
    ...
    <div className="scroll" style={{ flex: 1 }}>
      <div className="screen-pad" style={{ maxWidth: 1100, margin: "0 auto" }}>
        ...
        <div className="list-toolbar">
          ...
          {!isMobile && (
            <div style={{ position: "relative" }}>
              <button className="btn btn-sm btn-outline" onClick={() => setColsOpen((v) => !v)}>
          ...
          <button className="btn btn-sm btn-outline" onClick={() => onChat && onChat([])}>
            <Icons.Spark size={14} /> Chat with {meta.plural.toLowerCase()}
          </button>
          {canCreate && (
            <button className="btn btn-sm btn-primary" onClick={() => setCreateOpen(true)}>
              <Icons.Plus size={14} /> New {meta.label.toLowerCase()}
            </button>
          )}
        </div>
        ...
        {loading && rows.length === 0 ? (
          ...
          >
            <span className="spin" style={{ display: "inline-flex", marginBottom: 10 }}>
          ...
          >
            ...
            <div>
              {search
                ...
                : `No ${meta.plural.toLowerCase()} synced yet.`}
            </div>
          </div>
        ) : isMobile ? (
          <div className="ecard-list">
            {rows.map((r, i) => (
              <MobileCard
                key={r.id}
                onOpen={onOpen}
                onToggle={() => toggleRow(i, r.id, false)}
                rec={r}
          ...
          <div className={"etable" + (selCount > 0 ? " selecting" : "")}>
            <div className="ehead" style={{ gridTemplateColumns: grid }}>
              ...
              >
                <Check indeterminate={someOn} on={allOn} />
              </button>
              {shownCols.map((c) => (
                <div key={c[0]}>{c[1]}</div>
              ))}
            ...
            {rows.map((r, i) => {
              ...
              return (
                ...
                >
                  ...
                  {shownCols.map((c) => (
                    <Cell col={c} key={c[0]} onOpen={onOpen} rec={r} />
                  ))}
                  <div className="chev">
      ...
      {picker && (
        <ValuePicker
          ...
          onPick={(v) => runBulk(picker.action, v)}
          options={picker.options}
          title={picker.action.label}
        />
      )}
      {createOpen && (
        <CreateModal

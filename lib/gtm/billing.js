...
export function billingStatus() {
  ...
  if (CONFIG.billing.provider === "stripe") {
    ...
    if (live && live.state && live.state !== "none") return live;
  }
...
export async function refreshBillingStatus() {
  if (!CONFIG.billing.enabled || CONFIG.billing.provider !== "stripe") return billingStatus();
  const acct = getAccount();
  if (!acct?.email) return billingStatus();
  try {
    const s = await fetch(`/api/billing/status?email=${encodeURIComponent(acct.email)}`).then((r) => r.json());
    if (s && s.configured && s.state) write(STATUS_KEY, JSON.stringify(s));
  } catch {}
...
export function resetBilling() {
  [ACCOUNT_KEY, TRIAL_KEY, SUB_KEY, STATUS_KEY].forEach((k) => { try { localStorage.removeItem(k); } catch {} });
}

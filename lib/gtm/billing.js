...
// True when the workspace has an active paid (Pro) entitlement. Used to gate
// Pro-only features. Trialing / expired / none are NOT Pro.
export const isPro = () => billingStatus().state === "subscribed";

// Whether a Pro-only feature should be locked for this workspace. Only gates
// when billing is enabled — with billing off there are no tiers, so the
// feature stays open (template / self-host default).
export const isProFeatureLocked = () => CONFIG.billing.enabled && !isPro();

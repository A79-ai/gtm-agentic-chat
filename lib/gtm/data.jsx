...
export const CONNECTORS = [
  { id: "hubspot", name: "HubSpot", logo: "HubSpot", cat: "CRM", connected: true, desc: "Sync companies, deals, contacts and pipeline stages." },
  { id: "salesforce", name: "Salesforce", logo: "Salesforce", cat: "CRM", connected: false, desc: "Sync accounts, opportunities, leads and activities." },
  { id: "dynamics", name: "Dynamics CRM", logo: "Dynamics", cat: "CRM", connected: false, desc: "Sync accounts, opportunities and contacts from Microsoft." },
  { id: "gong", name: "Gong", logo: "Gong", cat: "Call Recording", connected: true, desc: "Pull call recordings, transcripts and deal signals." },
...
export const ENTITIES = {
  ...
  account: { type: "account", label: "Account", plural: "Accounts", icon: "Building", tone: "teal" },
  contact: { type: "contact", label: "Contact", plural: "Contacts", icon: "User", tone: "mint" },
  meeting: { type: "meeting", label: "Meeting", plural: "Meetings", icon: "Calendar", tone: "teal" },
...
export const AGENTS = [
  { id: "deal-coach", name: "Deal Coach", icon: "Target", tone: "gold", tag: "Pipeline", lastRun: "2h ago", starter: true, desc: "Chat with any deal. Pulls CRM, calls and notes to surface risks, momentum and the next best action.", tools: ["hubspot", "gong", "granola"] },
  { id: "call-digest", name: "Call Digest", icon: "Phone", tone: "teal", tag: "Calls", lastRun: "20m ago", starter: true, desc: "Summarize any call, extract commitments and objections, and draft the follow-up in seconds.", tools: ["gong", "fireflies"] },
  { id: "outreach", name: "Outreach Writer", icon: "Mail", tone: "mint", tag: "Prospecting", lastRun: "1d ago", starter: true, desc: "Draft personalized sequences grounded in real account research and recent conversations.", tools: ["hubspot", "slack"] },
  { id: "account-research", name: "Account Researcher", icon: "Building", tone: "gold", tag: "Research", lastRun: "3h ago", starter: false, desc: "Build a one-page brief on any account — org chart, signals, whitespace and warm paths in.", tools: ["hubspot", "devrev"] },
  { id: "pipeline-risk", name: "Pipeline Risk Scanner", icon: "Activity", tone: "teal", tag: "Pipeline", lastRun: "5h ago", starter: false, desc: "Scan the whole book for deals slipping past their kickoff window and explain why.", tools: ["hubspot", "gong"] },
  { id: "meeting-prep", name: "Meeting Prep", icon: "Calendar", tone: "mint", tag: "Calls", lastRun: "Yesterday", starter: false, desc: "Walk into every call ready — agenda, attendee context and the last three touchpoints.", tools: ["granola", "hubspot"] },
];

export const FIELDS = {
  deal: [["Stage", "stage", "badge"], ["Amount", "amount"], ["Close date", "closeDate"], ["Account", "accountName"], ["Owner", "ownerId", "ref"]],
...
export const COLUMNS = {
  deal: [["name", "Deal", "title"], ["stage", "Stage", "badge"], ["amount", "Amount", "text"], ["accountName", "Account", "text"], ["ownerId", "Owner", "avatar"]],
...
export const SUGGESTIONS = {
  deal: ["How is this deal progressing?", "What were the takeaways from the last meeting?", "What risks should I be aware of?", "Draft a follow-up email"],
  account: ["Summarize this account", "What's the whitespace here?", "Who are the key stakeholders?", "Draft an executive intro"],
  contact: ["Brief me on this person", "What's our relationship history?", "Draft an outreach message"],
  ...
  none: ["What needs my attention today?", "Which deals are at risk?", "Summarize my pipeline", "What meetings happened this week?"],
};

export function healthTone(v) {
  if (/risk|overdue|stalled|lost/i.test(v)) return "danger";
  if (/healthy|expanding|done|win|completed|closed won/i.test(v)) return "success";
  return "neutral";
}
...
export function setRecords(data) {
  for (const t of ENTITY_ORDER) STORE[t] = Array.isArray(data?.[t]) ? data[t] : [];
  reindex();
}
...
export function subtitleOf(r) {
  if (!r) return "";
  switch (r.type) {
    case "deal": return [r.stage, r.amount].filter(Boolean).join(" · ");
    case "account": return [r.industry, r.arr && `${r.arr} pipeline`].filter(Boolean).join(" · ");
    case "contact": { const a = byId(r.accountId); return [r.title, a && a.name].filter(Boolean).join(" · "); }
    case "meeting": return [r.date, r.durationMin && `${r.durationMin} min`].filter(Boolean).join(" · ");
    case "task": return [r.status, r.due && `due ${r.due}`].filter(Boolean).join(" · ");
    case "owner": return [r.role, r.region].filter(Boolean).join(" · ");
    default: return "";
  }
}
...
export function related(r) {
  const out = {};
  const push = (k, rec) => { if (rec) (out[k] = out[k] || []).push(rec); };
  if (r.type === "deal") {
    push("account", byId(r.accountId)); push("owner", byId(r.ownerId));
    ...
    push("account", byId(r.accountId)); push("deal", byId(r.dealId)); push("owner", byId(r.ownerId));
  } else if (r.type === "task") {
    push("owner", byId(r.ownerId)); push("deal", byId(r.dealId));
...
export function searchAll(q) {
  ...
  if (!q) return ALL;
  return ALL.filter((r) => (r.name || "").toLowerCase().includes(q) || subtitleOf(r).toLowerCase().includes(q));
}

export const DATA = {
  CONNECTORS, CAT_TABS, ENTITIES, ENTITY_ORDER, FIELDS, COLUMNS, SUGGESTIONS, AGENTS,
  byId, recordsOf, titleOf, subtitleOf, initials, related, searchAll, healthTone,
};

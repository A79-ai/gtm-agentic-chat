...
export const NORMALIZE: Record<string, (r: Rec, i: number) => Rec> = {
  ...
  contact: (c, i) => ({
    ...
    name:
      ...
      "Contact",
    title: s(c.title),
    accountId: s(c.account_id),
    accountName: s(c.company) || s(c.account_name),
    email: s(c.email),
    phone: s(c.phone_number) || s(c.phone),
    ownerId: s(c.owner_id),
  }),
};
...
export function pageOf(parsed: unknown): { items: Rec[]; total: number | null } {
  if (Array.isArray(parsed)) {
    return { items: parsed as Rec[], total: parsed.length };
  }

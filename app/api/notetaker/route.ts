...
export async function GET(req: Request) {
  ...
  const configs = await fetch(`${apiBase()}/api/v1/configs`, {
    headers: { Authorization: `Bearer ${key}` },
  })
    .then((r) => (r.ok ? r.json() : []))
    .catch(() => []);
  const val = (name: string) => {
    const c = Array.isArray(configs)
      ? (configs as Rec[]).find((x) => x.namespace === NS && x.name === name)
      : null;
    return c ? s(c.value) : "";
  };
  let emails: string[] = [];
  try {
    emails = JSON.parse(val("ampup_notetaker_enabled_user_emails") || "[]");
  } catch {
    emails = [];
  }
  return Response.json(
    {
      mode: val("ampup_notetaker_mode") || "disabled",
      name: val("ampup_notetaker_name") || "AmpUp Notetaker",
      enabledEmails: emails,
    },
    { headers: CORS }
  );
}
...
export async function POST(req: Request) {
  ...
  if (typeof body.mode === "string" && MODES.includes(body.mode)) {
    writes.push({ name: "ampup_notetaker_mode", value: body.mode });
  }
  if (typeof body.name === "string") {
    writes.push({ name: "ampup_notetaker_name", value: body.name });
  }
  if (!writes.length) {
    ...
    writes.map((w) =>
      fetch(`${apiBase()}/api/v1/config`, {
        ...
        body: JSON.stringify({
          ...
          name: w.name,
          value: w.value,
          reason: "GTM app notetaker settings",
          scope_key: "ORG",
          scope_value: "",
        }),
      }).then((r) => r.ok)

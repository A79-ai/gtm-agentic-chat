...
const MAP: Record<string, { tool: string; fields: string[]; required: string[] }> = {
  account: { tool: "create_account", fields: ["name", "industry", "sync_to_crm"], required: ["name"] },
  deal: {
...
function idOf(content: string): string {
  try {
    ...
    return String(o?.id ?? o?.account?.id ?? o?.opportunity?.id ?? o?.contact?.id ?? o?.task?.id ?? "");
  } catch {
    return "";
  }
}
...
export async function POST(req: Request) {
  ...
  const body = (await req.json().catch(() => ({}))) as { type?: string; fields?: Record<string, unknown> };
  const spec = body.type ? MAP[body.type] : undefined;
  if (!spec) return Response.json({ error: "unsupported type" }, { status: 400, headers: CORS });

  const input = body.fields || {};
  const missing = spec.required.filter((f) => input[f] == null || input[f] === "");
  if (missing.length) return Response.json({ error: `missing: ${missing.join(", ")}` }, { status: 400, headers: CORS });

  const args: Record<string, unknown> = {};
  for (const f of spec.fields) {
    if (input[f] != null && input[f] !== "") args[f] = f === "amount" ? Number(input[f]) : input[f];
  }
